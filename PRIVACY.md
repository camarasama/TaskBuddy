...
> and the UK Children's Code (Age Appropriate Design Code). Bracketed `[...]` items
> are decisions the provider and its counsel must confirm.
...
(see §5). TaskBuddy child profiles are intended for children aged **[10–16]**; we do not
...
Before a child profile is activated, the parent provides **verifiable parental consent**
using [the provider's chosen COPPA-compliant method — to be confirmed with counsel]. The
parent's consent is recorded (with the policy version consented to) at registration and at
each child profile's creation.
...
- **Hosting / database:** [OVH — EU region].
...
- **Payments:** [Stripe / Apple / Google] for the respective purchase channels.
...
We aim to store personal data in the **[EU/EEA]**. Where a processor transfers data outside the
EEA/UK, we rely on an appropriate safeguard such as an adequacy decision or Standard Contractual
Clauses. [Confirm each processor's transfer mechanism with counsel.]
...
TaskBuddy uses only **strictly necessary** cookies/local storage for sign-in and security. We do
not use advertising or cross-site tracking cookies. [Confirm the final cookie inventory.]
...
Data Protection Officer / EU-UK representative: [to be appointed if required].
