 ...
 * Overdue tasks with days-past-due indicator + expiry rate summary.
 */
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Overdue Tasks', value: report.summary.totalOverdue, color: 'text-orange-500' },
          { label: 'Expired (>1 day)', value: report.summary.totalExpired, color: 'text-red-600' },
          { label: 'Expiry Rate', value: `${report.summary.expiryRate}%`, color: 'text-gray-700' },
        ].map((s) => (
      ...
      {byChildData.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={byChildData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
              <XAxis dataKey="name" tick={{ fontSize: 11 }} />
              <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
              <Tooltip />
              <Bar dataKey="count" fill="#f97316" radius={[4, 4, 0, 0]} name="Overdue" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {report.rows.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 p-8 text-center text-green-600 font-medium">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50">
              <tr>{['Child', 'Task', 'Tag', 'Due Date', 'Status', 'Days Past Due'].map((h) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {report.rows.map((r, i) => (
                <tr key={i} className={`hover:bg-gray-50/50 ${(r.daysPastDue ?? 0) > 3 ? 'bg-red-50/30' : ''}`}>
                  ...
                  <td className="px-3 py-2 text-gray-500 capitalize">{r.status}</td>
                  <td className="px-3 py-2">
                    {r.daysPastDue !== null && r.daysPastDue > 0 ? (
                      <span className={`font-medium ${r.daysPastDue > 3 ? 'text-red-600' : 'text-orange-500'}`}>
                        {r.daysPastDue}d overdue
                      </span>
                    ) : <span className="text-amber-500 text-xs">due soon</span>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="flex gap-3">
        <a href={reportsApi.exportCsvUrl('expiry-overdue', { childId, startDate, endDate })} target="_blank" rel="noreferrer"
