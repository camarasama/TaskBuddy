...
const STATUS_COLORS: Record<string, string> = { pending: '#f59e0b', approved: '#6366f1', fulfilled: '#22c55e', cancelled: '#94a3b8' };
...
export default function RewardRedemptionReport({ childId, startDate, endDate }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setReport(await reportsApi.getRewardRedemption({ childId, startDate, endDate }) as Report); }
  ...
  const handleExport = async (format: 'csv' | 'pdf') => {
    ...
    try {
      const url = format === 'csv'
        ? reportsApi.exportCsvUrl('reward-redemption', { childId, startDate, endDate })
        : reportsApi.exportPdfUrl('reward-redemption', { childId, startDate, endDate });
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        {[{ label: 'Total Redemptions', value: report.summary.totalRedemptions, color: 'text-indigo-600' }, { label: 'Total Points Spent', value: report.summary.totalPointsSpent, color: 'text-red-500' }].map((s) => (
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
        ...
        <ResponsiveContainer width="100%" height={200}><BarChart data={report.summary.topRewards} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" /><XAxis type="number" allowDecimals={false} tick={{ fontSize: 11 }} /><YAxis dataKey="rewardName" type="category" width={150} tick={{ fontSize: 11 }} /><Tooltip /><Bar dataKey="count" fill="#6366f1" radius={[0, 4, 4, 0]} name="Times Redeemed" /></BarChart></ResponsiveContainer>
      </div>
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">By Status</h3>
          <ResponsiveContainer width="100%" height={180}><PieChart><Pie data={statusData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({ name, percent }) => `${name} ${((percent ?? 0) * 100).toFixed(0)}%`}>{statusData.map((d) => <Cell key={d.name} fill={STATUS_COLORS[d.name] ?? '#94a3b8'} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">By Reward Tier</h3>
          <ResponsiveContainer width="100%" height={180}><PieChart><Pie data={tierData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({ name, percent }) => `${name} ${((percent ?? 0) * 100).toFixed(0)}%`}>{tierData.map((d, i) => <Cell key={d.name} fill={TIER_COLORS[i % TIER_COLORS.length]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer>
        </div>
      </div>
        ...
        <table className="min-w-full text-sm"><thead className="bg-gray-50"><tr>{['Date', 'Child', 'Reward', 'Tier', 'Points', 'Status', 'Fulfilled'].map((h) => <th key={h} className="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-gray-50">{report.rows.slice(0, 50).map((r, i) => (
            <tr key={i} className="hover:bg-gray-50/50">
              <td className="px-3 py-2 text-gray-600">{r.date}</td><td className="px-3 py-2 font-medium text-gray-800">{r.childName}</td><td className="px-3 py-2 text-gray-600">{r.rewardName}</td><td className="px-3 py-2 text-gray-500 capitalize">{r.rewardTier ?? '—'}</td><td className="px-3 py-2 text-red-500 font-medium">{r.pointsSpent}</td>
