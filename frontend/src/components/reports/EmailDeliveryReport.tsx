 ...
 * Email logs aggregation — delivery rate, per-type breakdown, failure reasons.
 */
...
export default function EmailDeliveryReport({ familyId, startDate, endDate }: Props) {
  ...
  const statusPie = [
    { name: 'Sent', value: report.summary.totalSent },
    { name: 'Failed', value: report.summary.totalFailed },
    { name: 'Bounced', value: report.summary.totalBounced },
  ].filter((d) => d.value > 0);

  const triggerData = Object.entries(report.summary.byTriggerType).map(([name, { sent, failed }]) => ({
    name: name.replace(/_/g, ' '), sent, failed,
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Sent', value: report.summary.totalSent, color: 'text-green-600' },
          { label: 'Failed', value: report.summary.totalFailed, color: 'text-red-500' },
          { label: 'Bounced', value: report.summary.totalBounced, color: 'text-amber-500' },
          { label: 'Delivery Rate', value: `${report.summary.deliveryRate}%`, color: 'text-indigo-600' },
        ].map((s) => (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={triggerData} layout="vertical">
              ...
              <Tooltip />
              <Bar dataKey="sent" fill="#22c55e" radius={[0, 4, 4, 0]} name="Sent" stackId="a" />
      ...
      {report.summary.failureReasons.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          ...
          <div className="space-y-2">
            {report.summary.failureReasons.map((f, i) => (
              <div key={i} className="flex items-start justify-between gap-4 text-sm border-b border-gray-50 pb-2">
                <span className="text-gray-600 font-mono text-xs">{f.reason}</span>
                <span className="font-bold text-red-500 shrink-0">{f.count}×</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex items-center gap-3">
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
          className="rounded-lg border border-gray-200 text-sm px-3 py-1.5 bg-white text-gray-700">
          <option value="">All Statuses</option>
          <option value="sent">Sent</option>
          <option value="failed">Failed</option>
          <option value="bounced">Bounced</option>
        </select>
        <a href={reportsApi.exportCsvUrl('email-delivery', { familyId, startDate, endDate })} target="_blank" rel="noreferrer"
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr>{['Date', 'Trigger', 'Status', 'To', 'Subject', 'Resends'].map((h) => (
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.slice(0, 50).map((r, i) => (
              <tr key={i} className="hover:bg-gray-50/50">
                ...
                <td className="px-3 py-2 text-gray-600 text-xs">{r.triggerType.replace(/_/g, ' ')}</td>
                ...
                <td className="px-3 py-2 text-gray-600 text-xs">{r.toEmail}</td>
                <td className="px-3 py-2 text-gray-500 text-xs max-w-[200px] truncate">{r.subject}</td>
                <td className="px-3 py-2 text-gray-500 text-xs">{r.resendCount > 0 ? r.resendCount : '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {filtered.length > 50 && (
          <div className="px-4 py-3 text-sm text-gray-500 bg-gray-50 border-t">
            Showing 50 of {filtered.length} rows — export CSV for full data
          </div>
        )}
      </div>
    </div>
