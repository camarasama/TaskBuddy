 ...
 * Running balance line chart + transaction type breakdown.
 */
...
export default function PointsLedgerReport({ childId, startDate, endDate }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setReport(await reportsApi.getPointsLedger({ childId, startDate, endDate }) as Report);
  ...
  // Running balance line (last 50 entries reversed)
  const balanceData = [...report.rows].reverse().slice(0, 50).map((r, i) => ({
    i: i + 1,
  ...
  // Per-child earned vs spent
  const childData = Object.entries(report.summary.byChild).map(([name, { earned, spent }]) => ({
    name, earned, spent,
  }));
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Earned', value: report.summary.totalPointsEarned, color: 'text-green-600' },
          { label: 'Total Spent', value: report.summary.totalPointsSpent, color: 'text-red-500' },
          { label: 'Net Balance', value: report.summary.totalPointsEarned - report.summary.totalPointsSpent, color: 'text-indigo-600' },
        ].map((s) => (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={balanceData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
              <XAxis dataKey="i" tick={{ fontSize: 10 }} label={{ value: 'Transactions', position: 'insideBottom', offset: -4, fontSize: 10 }} />
              <YAxis tick={{ fontSize: 11 }} />
              <Tooltip />
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={typeData} layout="vertical">
              ...
              <Bar dataKey="value" radius={[0, 4, 4, 0]} name="Count">
                {typeData.map((d) => <cell key={d.name} fill={TYPE_COLORS[d.name] ?? '#94a3b8'} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="flex gap-3">
        <a href={reportsApi.exportCsvUrl('points-ledger', { childId, startDate, endDate })} target="_blank" rel="noreferrer"
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              {['Date', 'Child', 'Type', 'Amount', 'Balance After', 'Description'].map((h) => (
          ...
          <tbody className="divide-y divide-gray-50">
            {report.rows.slice(0, 50).map((r, i) => (
              <tr key={i} className="hover:bg-gray-50/50">
                ...
                <td className={`px-3 py-2 font-medium ${r.pointsAmount >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                  {r.pointsAmount >= 0 ? '+' : ''}{r.pointsAmount}
                </td>
                <td className="px-3 py-2 text-gray-700">{r.balanceAfter}</td>
