 ...
 * Full audit_logs with filters by action/resource + pagination.
 */
...
interface Report {
  rows: AuditRow[];
  total: number;
  summary: { byAction: Record<string, number>; byResourceType: Record<string, number> };
}
...
const ACTION_COLORS: Record<string, string> = {
  ...
  SUSPEND: 'bg-red-200 text-red-800',
};
...
export default function AuditTrailReport({ familyId, startDate, endDate }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setReport(await reportsApi.getAuditTrail({ familyId, startDate, endDate, page, pageSize: 50 }) as Report);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr>{['Timestamp', 'Actor', 'Action', 'Resource', 'Resource ID', 'IP'].map((h) => (
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.map((r) => (
              <tr key={r.id} className="hover:bg-gray-50/50">
                <td className="px-3 py-2 text-gray-500 text-xs whitespace-nowrap">{new Date(r.createdAt).toLocaleString()}</td>
                <td className="px-3 py-2 font-medium text-gray-700">{r.actorName ?? 'System'}</td>
                <td className="px-3 py-2">
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${ACTION_COLORS[r.action] ?? 'bg-gray-100 text-gray-600'}`}>
                    {r.action}
                  </span>
                </td>
                <td className="px-3 py-2 text-gray-600">{r.resourceType}</td>
                <td className="px-3 py-2 text-gray-400 font-mono text-xs">{r.resourceId.slice(0, 8)}…</td>
                <td className="px-3 py-2 text-gray-400 text-xs">{r.ipAddress ?? '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="flex gap-3">
        <a href={reportsApi.exportCsvUrl('audit-trail', { familyId, startDate, endDate })} target="_blank" rel="noreferrer"
