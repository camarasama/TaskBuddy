 ...
 * Admin-only. DAU/WAU/MAU + family growth + co-parent usage + task stats.
 */
...
export default function PlatformHealthReport() {
  ...
  const pdfUrl = reportsApi.exportPdfUrl('platform-health');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Families', value: report.userStats.totalFamilies },
            { label: 'Parents', value: report.userStats.totalParents },
            { label: 'Children', value: report.userStats.totalChildren },
            { label: 'New Families (month)', value: report.userStats.newFamiliesThisMonth },
          ].map((s) => (
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            {[
              ...
              ['Accepted', report.coParentStats.totalInvitesAccepted],
              ['Cancelled', report.coParentStats.totalInvitesCancelled],
            ].map(([label, val]) => (
            ...
            <div className="flex justify-between pt-1">
              ...
              <span className="font-bold text-indigo-600">{report.coParentStats.acceptanceRate}%</span>
            </div>
          </div>
          {relData.length > 0 && (
            <div className="mt-4">
              <div className="text-xs text-gray-500 mb-2">By Relationship Type</div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          ...
          <div className="space-y-3">
            {[
              { label: 'Tasks Created', value: report.taskStats.totalTasksCreated },
              { label: 'Assignments Approved', value: report.taskStats.totalAssignmentsApproved },
              { label: 'Avg Approval Time', value: `${report.taskStats.averageApprovalTimeHours}h` },
              { label: 'Active Families (week)', value: report.userStats.activeFamiliesThisWeek },
            ].map((s) => (
              <div key={s.label} className="flex justify-between border-b border-gray-50 pb-2 text-sm">
      ...
      <div className="flex gap-3">
        <a href={reportsApi.exportCsvUrl('platform-health')} target="_blank" rel="noreferrer"
