...
import {
  ArrowLeft, Save, Loader2, Clock, Tag, UserPlus, X, Repeat,
} from 'lucide-react';
...
const DIFFICULTY_OPTIONS: { value: FormState['difficulty']; label: string; active: string }[] = [
  { value: 'easy',   label: 'Easy',   active: 'border-green-400 bg-green-50 text-green-700' },
  { value: 'medium', label: 'Medium', active: 'border-yellow-400 bg-yellow-50 text-yellow-700' },
  { value: 'hard',   label: 'Hard',   active: 'border-red-400 bg-red-50 text-red-700' },
];
...
export default function EditTaskPage() {
  ...
  const [form,         setForm]         = useState<FormState>({
    ...
    difficulty: 'medium', taskTag: 'primary',
    pointsValue: '', dueDate: '', startTime: '',
    estimatedMinutes: '', maxClaimsTotal: '',
    requiresPhotoEvidence: false, status: 'active',
  ...
  const loadTask = useCallback(async () => {
    try {
      ...
      const t = (response.data as { task: Task }).task;
      setTask(t);
      setForm({
        title:                 t.title,
        ...
        status:                t.status === 'archived' ? 'active' : t.status,
  ...
  const handleUnassign = async (childId: string) => {
    setUnassigningId(childId);
    try {
      await tasksApi.unassignChild(taskId, childId);
      setTask((prev) =>
        prev
          ? { ...prev, assignments: prev.assignments?.filter((a) => a.child.id !== childId) }
          : prev
      );
        ...
        setTask((prev) =>
          prev
            ? {
                ...
                assignments: [
                  ...(prev.assignments ?? []),
                  { id: Date.now().toString(), status: 'pending', child },
                ],
              }
            : prev
        );
  ...
  const performSave = async (skipWarnings = false) => {
    ...
    const maxClaimsNum = form.maxClaimsTotal ? parseInt(form.maxClaimsTotal, 10) : undefined;
    if (form.maxClaimsTotal && (isNaN(maxClaimsNum!) || maxClaimsNum! < 1 || maxClaimsNum! > 100)) {
      showError('Max claims must be between 1 and 100');
      return;
    }
    ...
    try {
      const response = await tasksApi.update(taskId, {
        ...
        maxClaimsTotal:        maxClaimsNum ?? null,
  ...
  const activeAssignments = (task.assignments ?? []).filter((a) =>
    ['pending', 'in_progress'].includes(a.status)
  );
  const assignedChildIds = new Set(activeAssignments.map((a) => a.child.id));
  const availableToAdd   = familyChildren.filter((c) => !assignedChildIds.has(c.id));
        ...
        >
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2 flex items-center gap-1.5">
              <Tag className="w-4 h-4" /> Task Type
            </label>
            <div className="flex gap-3">
              {(
                ...
                <button
                  key={opt.value}
                  type="button"
                  onClick={() => setForm((p) => ({ ...p, taskTag: opt.value }))}
                  className={cn(
            ...
            <div className="flex gap-3">
              {DIFFICULTY_OPTIONS.map((opt) => (
                <button
                  key={opt.value}
                  type="button"
                  onClick={() => setForm((p) => ({ ...p, difficulty: opt.value }))}
                  className={cn(
                    'flex-1 py-2 rounded-lg border-2 text-sm font-medium transition-all',
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              How many different children can claim this from the pool. Leave blank for unlimited.
            </p>
          </div>
              ...
              {activeAssignments.length === 0 ? (
                ...
                activeAssignments.map((a) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      disabled={unassigningId === a.child.id}
                      onClick={() => handleUnassign(a.child.id)}
                      className="hover:text-red-500 ml-0.5 disabled:opacity-40"
                    >
                      {unassigningId === a.child.id
            ...
            {availableToAdd.length > 0 && (
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={!addingChild || isAssigning}
                  loading={isAssigning}
                >
                  Assign
                </Button>
